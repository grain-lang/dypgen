         ...
         let c =
           if typ = "No_type" then
             (let c = if c="" then c else
              ...
              in
              "(fun " ^ (let contains_lexbuf =
                           try 
                             ignore (Str.search_forward (Str.regexp "\\blexbuf\\b") (c^obj_pref^cons) 0);true
                           with Not_found -> false
                         in
                         if contains_lexbuf then "lexbuf" else "_") ^ " -> "^c^obj_pref^cons^")")
           else if c="" then
            ...
            if code = "" then code_iv else
              ...
              (String.concat ""
                 [(string_of_int n')^",\n(fun __dypgen_ol __dypgen_pos";
                  ...
                  "(fun "; 
                  (let inh_val_contains_dyp =
                     try 
                       ignore (Str.search_forward (Str.regexp "\\bdyp\\b") (String.concat " " [ code_var_list;obj_pref; (String_map.find nt inh_cons_map);code]) 0);true
                     with Not_found -> false
                   in
                   if inh_val_contains_dyp then "dyp" else "_")
        ...
        (if b then
           ["Dyp.Dypgen_action (fun __dypgen_ol __dypgen_pos __dypgen_posl";
            ...
            (let action_contains_dyp =
               try 
                 ignore (Str.search_forward (Str.regexp "\\bdyp\\b") (String.concat " " [ code_var_list;action;obj_pref; (String_map.find lhs_nt symb_cons_map);]) 0);true
               with Not_found -> false
             in
           ...
           ["Dyp.Dypgen_action (fun __dypgen_ol __dypgen_pos __dypgen_posl";
            ...
            (let action_contains_dyp =
               try
                 ignore (Str.search_forward (Str.regexp "\\bdyp\\b") (String.concat " " [ code_var_list;obj_pref;(String_map.find lhs_nt symb_cons_map);action]) 0);true
               with Not_found -> false
             in
             if action_contains_dyp then "dyp" else "_");
            " __dypgen_av_list -> (match (";
